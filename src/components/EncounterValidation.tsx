...
function validateEncounter(encounter: Encounter): Validation[] {
    ...
    if (!encounter.class) {
        ...
        if (encounter.class.system !== 'http://terminology.hl7.org/CodeSystem/v3-ActCode') {
            newValidations.push(new Validation('Class system is not http://terminology.hl7.org/CodeSystem/v3-ActCode', Severity.ERROR))
        }
        if (!encounter.class.code) {
            ...
            newValidations.push(new Validation(`Class object code must be AMB, VR, but was "${encounter.class.code}"`, Severity.ERROR))
        }
    }
    ...
    if (!encounter.participant || encounter.participant.length === 0) {
        ...
        encounter.participant.forEach((participant) => {
            if (!participant.individual) {
                ...
                newValidations.push(new Validation('Participant individual reference is not of type Practitioner', Severity.ERROR))
            }
        })
    }
    ...
    if (!encounter.diagnosis || encounter.diagnosis.length === 0) {
        ...
        encounter.diagnosis.forEach((diagnosis) => {
            if (!diagnosis.condition) {
                ...
                newValidations.push(new Validation(`Diagnosis condition type is not set to "Condition", but was: ${diagnosis.condition.type}`, Severity.ERROR))
