...
export default function ConditionValidation({client}: ConditionValidationProps) {
    ...
    return (
        <div className="basis-1/5">
            ...
            {error ? (
                <ValidationTable validationTitle={'Condition validation'} validations={
                    [new Validation(handleError('Unable to fetch Condition', error), Severity.ERROR)]
                }/>
            ) : (
...
function validateCondition(condition: Condition): Validation[] {
    ...
    if (!condition.subject) {
        ...
        newValidations.push(new Validation(`The Condition subject must be of type Patient, but was "${condition.subject.type}"`, Severity.ERROR))
    }
    ...
    if (!condition.code) {
        ...
        if (validCodings.length === 0) {
            newValidations.push(new Validation('The Condition code object does not contain a valid coding reference', Severity.ERROR))
        }
